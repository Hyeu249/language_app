...
class LearningApp(models.Model):
    ...
    is_started = fields.Boolean("Is started", compute="get_is_started", tracking=True)
    ...
    started_learning_ids = fields.One2many(
        "lang.app.started.learning", "learning_id", string="Started learning"
    )
    ...
    @api.depends("started_learning_ids")
    def get_is_started(self):
        for record in self:
            started_id = record.started_learning_ids.filtered(
                lambda e: e.user_id == self.env.user and e.learning_id == record
            )

            record.is_started = started_id.is_started
    ...
    def set_is_started(self):
        self.ensure_one()
        started_id = self.find_or_create_started_learning()
        started_id.is_started = True

    def find_or_create_started_learning(self):
        self.ensure_one()
        started_id = self.started_learning_ids.search(
            [
                ("learning_id", "=", self.id),
        ...
        if started_id:
            ...
            return self.started_learning_ids.create(
                {
                    "learning_id": self.id,
                    "user_id": self.env.user.id,
                }
    ...
    def get_started_learnings(self):
        started_learnings = self.started_learning_ids.search(
            [("user_id", "=", self.env.user.id), ("is_started", "=", True)]
        )
        learning_ids = started_learnings.mapped("learning_id")
...
class Vocabulary(models.Model):
    ...
    def get_paths(self):
        self.ensure_one()
        app_path = "/opt/bitnami/odoo/addons/"
...
class StartedLearning(models.Model):
    _name = "lang.app.started.learning"
    _description = "Lang app started learning records"
    _inherit = ["mail.thread"]
    _check_company_auto = True

    is_started = fields.Boolean("Is started", tracking=True)

    # related
    name = fields.Char("Name", related="learning_id.name", tracking=True)
    ...
    learning_id = fields.Many2one("lang.app.learning", string="Learning")

    # sequential
    ref = fields.Char(string="Code", default=lambda self: _("New"))

    @api.constrains("user_id", "learning_id")
    def _unique_user(self):
        for record in self:
            duplicate = self.search(
                [
                    ("id", "!=", record.id),
                    ("user_id", "=", record.user_id.id),
                    ("learning_id", "=", record.learning_id.id),
                ]
            )
            message = "Trùng người dùng ở đã học learning!"
            if duplicate:
                raise ValidationError(message)

    @api.model_create_multi
    def create(self, vals_list):
        result = super(StartedLearning, self).create(vals_list)

        return result

    def write(self, vals):
        self.ensure_one()
        result = super(StartedLearning, self).write(vals)
        return result
